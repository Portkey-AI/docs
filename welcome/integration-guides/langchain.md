...
**LangChain** is a framework for developing applications and agents powered by language models. It enables applications that:
...
```bash
pip install -U langchain-core portkey_ai
```
...
Let's import the Langchain x OpenAI integration package
```bash
...
When the LLM is invoked, Portkey will distribute the requests to `gpt-4` and `claude-opus-20240229` in the ratio of the defined weights.
